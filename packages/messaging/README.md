...
queries several drivers with a per-driver deadline and returns partial results.

Everything here must survive serialise/deserialise — that is the property test, and anything that does
not is not a valid message. Deadlines live in the envelope, so RCD-11 cannot be forgotten:
there is no way to send a request without one.
...
and a public surface is how internal metadata reaches a compat shape (RCD-10).
