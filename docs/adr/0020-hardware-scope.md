# ADR-0020 — Hardware scope: Patron, Neuron, Unipi 1.1, Extensions, Gate; Edge as a fast follow; Axon dropped

- **Status:** Accepted
- **Date:** 2026-08-12
- **Refs:** docs/research/05-evok-node-design-notes.md §8.3, §7.1 ·
  docs/research/09-test-hardware-coverage.md §5 · docs/GOALS.md §Non-goals · ADR-0016, ADR-0023
...
**Two obligations land now rather than with the models.** Deferring either would mean a breaking
change in the first minor release after 1.0:

- The overlay format (ADR-0016) must already accommodate Edge's requirements — per-channel mode sets,
  per-model mode enums (4–20 mA, 90–2000 Ω against the PLC families' 0–20 mA and 0–1960 Ω), and unit-0
  devices.
- Family forks are **capability flags derived from definition plus identity**, never model-name
  ...
  `hasUnitZeroAggregate`, `aiAbstraction`, `hasBoardFirmware`. See
  [research/05 §7.1](../research/05-evok-node-design-notes.md).
...
simulator-verified until purchased (ADR-0023), and Unipi 1.1 is entirely unverified — `unipi-one-modbus`
on port 50200, no board firmware, no NV save, 18-bit AI. research/09 §5 puts a Neuron **L203** and a
Unipi 1.1 at purchase priorities 2 and 3 for that reason.

**Rejected: Edge inside 1.0.** It is structurally different — slot/card identity, unit-0 ULED,
per-channel AI modes — and EVOK claims it nowhere, so including it would add scope to the one thing
1.0 is defined as. Its maps are the best-documented in the corpus, which makes it tempting and does
not make it in scope.

**Rejected: dropping Edge entirely.** Its product page does list EVOK support even though EVOK's
README does not mention it, and it is the current flagship. Being format-ready costs one design pass
now; retrofitting a definition format costs a breaking release.

**Rejected: supporting Axon because the maps are already there.** A claim of support is a claim to
test, on hardware we will never buy for a discontinued line. Keeping the CSVs as cross-check data
