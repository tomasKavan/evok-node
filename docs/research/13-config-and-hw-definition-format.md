...
**Status: accepted 2026-08-13 as [ADR-0014](to_revision/0014-hardware-definitions-and-inventory-are-ours.md),
which supersedes ADR-0007.** This file is the format specification the ADR points at; the ADR carries the
decision and the rejected alternatives. §8's upstream changes have been **applied** — it is kept as the
...
## 8. What this changed upstream — applied 2026-08-13

Not a wording pass. **We now read nothing from `/etc/evok` and depend on no Unipi data package**, so
every place that describes platform facts as EVOK's files read in place becomes wrong, not merely
imprecise.

| where | change |
|---|---|
| **G-5** platform-facts row | Written by **us alone** — our package ships definitions, our generator writes `autogen.yaml`. Delete "OS image paths as EVOK reads them", delete "overlay", drop `unipi-os-configurator` from the written-by column. The sentence "We read EVOK's, in EVOK's format and in place, unaffected by ADR-0006's migration" goes entirely. "Never the daemon" and "frozen per load" (RC-4) survive unchanged |
| **ADR-0007** | Superseded. The overlay layer, three-layer merge, per-field provenance, `appliesTo` fingerprints and the warn-in-theirs/error-in-ours asymmetry all disappear with it |
| **ADR-0005** | **Delete its copy of the lifecycle table** and cite G-5 instead — see the RD-6 note below. What stays is what only an ADR can say: finding 3.9 as context, SQLite via `node:sqlite` in `driver-store` rather than `main`, export/import, consequences, rejected alternatives. Its platform-facts paragraph ("we **extend** the definition format by overlay") goes with ADR-0007 |
| **ADR-0006** | `Depends:` must name **`nginx`** — `apt-get remove --auto-remove evok` takes `nginx` and `nginx-common` with it. And the `unipi-kernel-modules` worry is unfounded: `evok` declares `Depends: python3` and nothing else, so removing it cannot autoremove them (RD-7, dated). No dependency on `evok-unipi-data` is needed after all |
| **ADR-0012** | Unchanged in mechanism, extended in purpose: the script-generated address tables now also **cross-check the definitions** (§5.1). Its "hand-written expected-value tables drift silently" rejection still holds — the tables stay generated; it is the definitions that are hand-maintained, and the tables are what stop them drifting |
| **research/03** | The 2026-08-10 clarification — "§2 `autogen.yaml` and §4 `hw_definitions` remain runtime daemon inputs, read from the OS image as EVOK reads them. Unchanged." — is superseded. §4 stays valuable as the parsing spec for the *migration* path and as cross-check data (RD-7, dated) |
| **research/05 §8.5** | The three-layer overlay design is superseded; §2.5's "our own autogen equivalent" is now the whole story rather than a fallback |
| **research/12** | Two new config-parse checks (§1), and `model:` → `definition:` in the config sketch |
| **research/02 §3.3** + raw appendix | Register 1002's bit order is backwards in both; §2.1 depends on the corrected reading (RD-7, dated) |
| **capture-trip.md** | Drops the `evok-unipi-data` questions. Adds: which of the two `run.d` directories exists and what owns it, and register 1004 per unit to seed `identifies.hardwareId` (line 68 already collects it). **Promotes the stock `hw_definitions` copy from useful to blocking** — §5.1 needs its `modes{}` before board `00` can be written at all |
| **STATUS.md** | N1 scope grows to include definition generation; the ADR-0006 packaging blockers shrink to `nginx` plus the systemd/nginx site facts (RP-1, same PR) |

### 8.1 G-5 and ADR-0005 state the same rule twice — an RD-6 bug, and it has already drifted

G-5's invariant and ADR-0005's Decision are both titled "four kinds of data, four lifecycles" and carry
**the same four-row table with the same three columns**, plus the same "the daemon never writes its config
file" and "frozen per load, not once per process". RD-6: a rule stated in two places is a bug.

This is not hypothetical tidying. The two copies have already diverged in wording — ADR-0005's
platform-facts row says "EVOK's paths in EVOK's format, read in place; ours alongside (ADR-0007)" where
G-5 says "OS image paths as EVOK reads them, plus our own overlay and cache paths". ADR-0005 is dated
2026-08-10 and G-5 carries a correction dated 2026-08-12, so one copy was edited and the other was not.
The §8 table above is the proof of cost: without this fix it would instruct an editor to make the same
change in two files and hope they match.

**The table lives once, in G-5** — highest precedence, and a lifecycle taxonomy is an invariant, not a
decision. ADR-0005 keeps what an ADR is for and what nothing else states: EVOK's 300 s alias flush
(finding 3.9) as the context, SQLite via `node:sqlite`, the store as `driver-store` rather than `main`,
export/import as a first-class requirement, and the rejected alternatives. Its title loses the first half.

Fixable independently of this proposal, and probably should be — it is a one-file deletion plus a citation,
and it stops us editing the platform-facts row twice.

Worth a separate look, not blocked on this: `60-evok-autogen.sh` has no `hw_data` entry for **Gate** or
**CM40**, and **Iris** is slot-cards rather than sections. ADR-0008's hardware scope should say
explicitly which of those we support.
