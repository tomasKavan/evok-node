...
**Decided:** 2026-08-10. Amended only by a dated note.
...
We are oferring EVOK *interface* as compat api, but we are not inheriting the EVOK *design*.
...
> **To be reviewed, and then moved to a better document.** This section predates the split between
> goals, decisions and design, and it is doing all three jobs at once: G-1 and G-5 are architecture,
> G-3 and G-7 are product promises, and the corrections stacked on them are a change log. Read them as
> the current best statement of what must not be retrofitted — but expect them to be rewritten and
> redistributed during the development-documentation pass, and do not add to them here.
