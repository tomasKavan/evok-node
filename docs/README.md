# evok-node documentation - how to use anad maintain it

## Layout

1. **[`GOALS.md`](docs/GOALS.md)** - Goals, non-goals, invariants (G-N). Authoritative on scope.
2. **[`Research`](docs/research)** - what is true about EVOK and Unipi hardware, and everything not yet decided: to_revision/ holds the suspended ADRs. Laboratory for further decisions. 
3. **[`Dev docs`](docs/dev)** - The design. Input for coders to build the software. How each part is built and why, 00-20 in reading order. Written from research. Superseed research findings and decision. Every divergence from research should be reasoned. This is what you implement against. 
4. **[`Rules`](docs/rules)** - How we work — the binding rules, plus per-package rules in rules/packages/
5. **[`Plan`](docs/plan)** - What we are doing next: STATUS.md, roadmap.md, and the RPL rules
6. **[`Modbus Registers Map`](docs/modbus-reg-map)** - official Unipi register maps — ground truth, read-only, no prose
