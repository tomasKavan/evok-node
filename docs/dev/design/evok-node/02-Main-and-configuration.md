...
A `device` kind declares itself the same way, resolving to a registered `DeviceType` (05a §3.3) instead of a `ModuleDescriptor` — but its own identifying field is named `deviceKind`, not `typeName`: unlike a `ModuleDescriptor`, a `DeviceType` already carries its own `kind` (05a §3.3), and this field's whole job is being checked against it, not just naming a slot (05a §3.5). A package bundling a whole device family behind one entry point names each one with `export`:
