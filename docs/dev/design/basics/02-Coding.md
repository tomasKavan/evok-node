...
[R ??] **If a rule can be a lint rule, it must be a lint rule**
...
[R ??] **Brand every number that means something**
...
[R ??] **Discriminated unions, not optional fields.**
...
[R ??] **`readonly` by default, and anything loaded from outside is frozen once loaded**
...
[R ??] **A decode returns a value on every path**
`decode(input) => Output`. Never mutate state in place and fall through.

## 2. Errors

[R ??] **Expected failure is a value; a bug is a `throw`.** 
...
[R ??] **A protocol-level error response is a failure.** 
...
[R ??] **No empty `catch {}` on an event or delivery path.** 
Log and count, with dedup.

[R ??] **Error kinds are string-literal unions, not classes.** 
...
[R ??] **No `sleep()` as a synchronisation primitive, and no `process.env` outside the config
module** 
...
[R ??] **Prefer a widely used, tested, maintained library** 
...
[R ??] **Comment the why, never the what — and as briefly as it can be said** 
